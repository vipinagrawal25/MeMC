...
int main(int argc, char *argv[]){
    ...
    for(int cycle = 0; cycle<4; cycle++){
        ...
        // --------------------------------------------------
        // Compute how many anneal steps needed to reach 1e-6
        // --------------------------------------------------
        int anneal_steps = 0;
        double temp = mcobj.getkBT();   // starting T of this cycle
        while (temp >= 1e-6){
            anneal_steps++;
            temp *= 0.1;
        }
        // --------------------------------------------------
        // Annealing loop (cycle-dependent length)
        // --------------------------------------------------
        for(int anneal = 0; anneal < anneal_steps; anneal++){
            mcobj.updateparam(anneal, outfolder);
            // --------------------------------------------------
            // MC iterations at fixed temperature
            // --------------------------------------------------
            int block_start = residx;
            ...
            if(iter % mcobj.dumpskip() == 0){
                ...
                if (mesh.ncomp > 1)
                    hdf5_io_write(mesh.compA, mesh.N, outfile, "lip");
